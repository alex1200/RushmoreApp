...
function iScroll (el, options) {
	...
	that.scale = that.options.initZoom;
	...
	if(that.options.verticalcenter) {
		that.scroller.style.cssText += '-webkit-transition-property:-webkit-transform;-webkit-transform-origin-x:0;-webkit-transform:' + trnOpen + '0,0' + trnClose;
	} else if (that.options.HWCompositing) {
		...
		if (!that.moved) {
			if (hasTouch) {
				if (that.doubleTapTimer && that.options.zoom) {
					...
					that.zoom(that.pointX, that.pointY, that.scale == that.options.zoomMax ? that.options.zoomMin : that.options.zoomMax);//LANDON
				} else {
...
/*			if (newPosX > 0 || newPosX < that.maxScrollX || newPosY > 0 || newPosY < that.maxScrollY) {
	...
	_gestEnd: function (e) {
		...
		//$('#output').html(that.scale+"asdf");
		//$('.hotspot').attr('width',1/that.scale*50);//LANDON
	...
	refresh: function (zoomlevel) {
		...
		//landon hack
		if(zoomlevel)that.scale=zoomlevel
	...
	scrollByOffset: function (offsetX, offsetY, time){//landon function
		var that = this, x, y;
		y=that.y+offsetY;
		x=that.x+offsetX;
	...
	zoom: function (x, y, scale) {
		...
		//$('.hotspot').attr('width',1/scale*50);//LANDON
